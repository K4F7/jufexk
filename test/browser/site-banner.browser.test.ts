...
test("shows the matching desktop or mobile site banner below the header @mobile-smoke", async ({
  ...
  await expect(banner.getByRole("button", { name: "关闭公告" })).toBeVisible();
});

test("closing the site banner hides it until the banner is updated", async ({
  page,
}) => {
  await page.route("**/api/**", async (route) => {
    const pathname = new URL(route.request().url()).pathname;
  ...
  const banner = page.getByRole("region", { name: "全站公告" });
  await expect(banner).toBeVisible();
  await banner.getByRole("button", { name: "关闭公告" }).click();
  await expect(banner).toHaveCount(0);

  await page.reload();
  await expect(page.getByRole("region", { name: "全站公告" })).toHaveCount(0);
});
