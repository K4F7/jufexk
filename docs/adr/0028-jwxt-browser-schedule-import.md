...
学生在自己的浏览器打开官方通道2（`https://jwxt.jxufe.edu.cn/jxcjcaslogin`）或 ehall「本科教务」。教务 `CASTGC` / `JSESSIONID` 只存在于教务站点。本站只接受书签或粘贴回传的课程名、教师名、周次与上课时间，写入本机排课计划，不把开课班写入公开目录，也不在 Worker 上代持或落库教务 Cookie。
