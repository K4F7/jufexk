...
- 已选课程以当前 `S2020302` 结果页为准；解析器兼容双层表头、方括号课程号、`上课班级` 和合并的时间/地点文本。该页缺少学期、年级或专业下拉项时允许空筛选维度，不伪造学校代码。`S2020103` 必须先选年级和专业再导出或浏览计划内/公共选修；「请选择…」占位项不是筛选维度。`/schedule` 用选课列表 + 开课班两栏和「选择课程」弹层处理候选课，不另做手机布局。
