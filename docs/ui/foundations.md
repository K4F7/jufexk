...
- **课程详情页（产品意向）**：可砍 / 低优先级。若目录行仍可进入 `/courses/:id`，也以导航落点为主，不作为本批 UI 验收重点。若日后保留该页，顶部摘要取 Prototype **B（左身份 / 右评分）**，见冻结表；**不**把原型直接并入生产。
- 课程详情和教师详情若存在，均采用单页纵向信息架构，不使用 Dashboard 式多张统计卡或不必要的 Tabs。
- 课程顶部（若保留）使用紧凑摘要区，包含课程名、课号、类别、院系、任课教师、平均评分和学生投稿数。
- 总体评分使用醒目的数字，例如 `4.7 / 5`，并同时显示学生投稿数；不以星阵、环形图或进度条为主。
- 学生投稿使用紧凑纵向条目和分隔线，直接展开评分、教师、学期、正文和评价维度；维度以 HeroUI `Chip` `variant=soft` 胶囊行展示（见冻结表）。
- 历史文字资料必须保持独立区块，不包含 overall，也不参与评分、学生投稿数或排序统计；空态文案为「暂无历史资料」，与投稿空态「暂无投稿」区分。
- **教师详情（视觉冻结 · 模块 11）**：单页纵向，沿用课程详情冻结语言，**不单开 A/B/C**。
  - **摘要 B**：左身份（姓名 / 职称 / 院系 / 任课门数 / 简介）· 右 Surface 大号评分 `x / 5` + 投稿数。
  - **任课课程表**：课程域折叠列（课名+类别 Chip · 课号次行 / 院系 / 评分·投稿）；真实课程链接；实现见 `TeacherCourseTable`。
  - **相关投稿 / 历史资料**：对齐模块 10（`ReviewCard` 身份行显示课程名；`LegacyReviews showCourse`）。
  - 返回教师目录保留 URL 状态。
