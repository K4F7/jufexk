...
- **课程详情页评价流（2026-08-18 起，Issue #201/#202 修订）**：区块为摘要 → 任课教师网格 → 评价流。未选教师时评价流展示该课全部公开评价（不再留白/引导空态）；点击卡片 Footer「查看评价」（真实链接，URL `?teacher=<id>` 可分享、可新标签打开）就地筛选，Footer「取消选择」去掉参数回到全部评价，切换教师命中会话内缓存即时恢复；教师主页仅经 Footer「教师主页」链接进入。提示置于教师网格上方，文案不使用「×」。课程评价 API：`/api/courses/:id` 只返回课程、教师列表与评价总数；`/api/courses/:id/reviews` 带 `teacherId` 时按 课程×教师 过滤，不带时返回该课全部公开评价。官方网格在窄屏（<sm）单列堆叠，不再另做自定义卡片（Issue #205）。
