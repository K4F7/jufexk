...
- **课程详情页不直接展示评价流（2026-08-17 起）**：区块为摘要 → 任课教师表；评价必须按 课程×教师 查看——点击教师行（真实链接，URL `?teacher=<id>` 可分享、可新标签打开，再次点击取消选择）后，该课程在该教师下的评价流才出现在教师表下方；未选择且有评价时显示引导空态，课程无评价时仍为「暂无评价」。教师姓名链接仍进入教师详情页。课程评价 API 同步收紧：`/api/courses/:id` 只返回课程、教师列表与评价总数，`/api/courses/:id/reviews` 必须带 `teacherId`。
