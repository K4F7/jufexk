...
- 课程顶部使用紧凑摘要区：左身份（类别 Chip、课程名、课号、院系），右评价数 Surface（公开文字评价数，无课程级评分）；任课教师为两列 Card 网格（Avatar 首字 · 姓名 · 有则显示院系/评分/`N 投` Chip），按任课关系展示评分/投稿（Issue #221）。
- **课程详情页不直接展示评价流（2026-08-17 起）**：区块为摘要 → 任课教师网格；评价必须按 课程×教师 查看——点击教师卡片（真实链接，URL `?teacher=<id>` 可分享、可新标签打开，再次点击取消选择）后，该课程在该教师下的评价流才出现在教师网格下方；未选择且有评价时显示引导空态，课程无评价时仍为「暂无评价」。教师详情通过卡片内「教师主页」链接进入。课程评价 API 同步收紧：`/api/courses/:id` 只返回课程、教师列表与评价总数，`/api/courses/:id/reviews` 必须带 `teacherId`。
...
| 课程详情摘要 | 视觉冻结 | **B — 左身份 / 右评价数**：类别 Chip + 课程名 + 课号/院系 · 右 Surface 仅显示公开评价数（#140 移除课程级评分数字）；下接「任课教师」两列 Card 网格（#221，评分按 教师×课程 展示；无院系/评分/投稿不渲染占位） | 评价必须绑定课程+任课教师（见 CONTEXT）；#115 明确重开并落地生产，按任课关系拆分评分比课程聚合更实用；#140 进一步明确课程界面不出现评分；#221 将密表改为官方 Card 网格以消化长名单空数据噪音 | 生产：`CourseDetailPage.tsx` · `DetailSummary.tsx` · `CourseTeacherTable.tsx` · 原型：`src/prototype/CourseDetailSummaryVariants.tsx` B · Issue #60 → #115 → #140 → #221 |
