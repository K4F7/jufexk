...
To capture a fresh cookie locally, run `pnpm run jwxt-cookie-capture`. The script opens only the eHall home page; the operator handles every login and subsequent navigation manually. Before writing `.env.jwxt-sync`, it runs a local `pilot` smoke test and saves the cookie only when at least one offering is captured. It never prints or uploads browser state.
