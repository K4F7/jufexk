...
`main` 部署还会 `wrangler deploy --env preview`，把同一构建发到 `jufexk-preview.<account>.workers.dev`。预览用独立 D1 `jufexk-preview`，不绑生产 AI 摘要队列。首次需要 `pnpm db:create-preview` 写入 `database_id`。要让预览看起来像生产目录，运维在本机执行 `pnpm db:clone-preview`（会先清空预览库对象再导入完整 dump，保留同一 `database_id`，不必先 apply migrations；体积可能很大，失败可重试）。Turnstile widget 需允许该预览主机名。
