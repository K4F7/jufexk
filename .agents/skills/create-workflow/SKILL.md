...
This is a project skill. Cursor Cloud Agents scan it at conversation start (`/create-workflow`); a session already running will not pick it up until restarted.

If this session has no `workflow` tool, still author the script and save it to `.grok/workflows/<name>.rhai`. Skip live `validate_only` / run, and report that only the file was written.
