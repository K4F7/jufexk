CREATE TABLE teachers_identity_new(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  source_teacher_label TEXT NOT NULL UNIQUE CHECK(length(trim(source_teacher_label))>0),
  name TEXT NOT NULL,
  department TEXT,
...
INSERT INTO teachers_identity_new(id,source_teacher_label,name,department,title,bio,created_at)
SELECT id,name,name,NULLIF(department,''),title,bio,created_at FROM teachers;

CREATE TABLE courses_identity_new(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  code TEXT NOT NULL UNIQUE CHECK(length(trim(code))>0),
  name TEXT NOT NULL,
  category TEXT NOT NULL CHECK(category IN('major','pe','general')),
  department TEXT NOT NULL DEFAULT '',
  credits REAL,
  description TEXT NOT NULL DEFAULT '',
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
INSERT INTO courses_identity_new(id,code,name,category,department,credits,description,created_at)
SELECT id,code,name,category,department,credits,description,created_at FROM courses;

CREATE TABLE catalog_identity_backfill_audit(
...
INSERT INTO catalog_identity_backfill_audit(entity_type,entity_id,previous_name,previous_department,source_identity)
SELECT 'teacher',id,name,department,name FROM teachers;
INSERT INTO catalog_identity_backfill_audit(entity_type,entity_id,previous_name,previous_department,source_identity)
SELECT 'course',id,name,department,code FROM courses;
...
ALTER TABLE teachers RENAME TO teachers_identity_legacy;
ALTER TABLE teachers_identity_new RENAME TO teachers;
ALTER TABLE courses RENAME TO courses_identity_legacy;
ALTER TABLE courses_identity_new RENAME TO courses;

ALTER TABLE course_teachers RENAME TO course_teachers_identity_legacy;
CREATE TABLE course_teachers(
  course_id INTEGER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
  teacher_id INTEGER NOT NULL REFERENCES teachers(id) ON DELETE CASCADE,
  PRIMARY KEY(course_id,teacher_id)
);
INSERT INTO course_teachers SELECT course_id,teacher_id FROM course_teachers_identity_legacy;

ALTER TABLE offerings RENAME TO offerings_identity_legacy;
...
SELECT id,course_id,term,section,campus,schedule,status,created_at FROM offerings_identity_legacy;

ALTER TABLE reviews RENAME TO reviews_identity_legacy;
...
SELECT id,course_id,teacher_id,category,attendance,grading,workload,rescue,assessment,teaching,clarity,knowledge,overall,comment,term,status,moderator_note,submitter_hash,created_at,reviewed_at,grading_score,offering_id,interest,practicality,workload_score,fairness,organization FROM reviews_identity_legacy;

ALTER TABLE review_moderation_events RENAME TO review_moderation_events_identity_legacy;
...
SELECT id,review_id,action,note,created_at FROM review_moderation_events_identity_legacy;

ALTER TABLE offering_teachers RENAME TO offering_teachers_identity_legacy;
...
INSERT INTO offering_teachers SELECT offering_id,teacher_id FROM offering_teachers_identity_legacy;

ALTER TABLE legacy_reviews RENAME TO legacy_reviews_identity_legacy;
...
INSERT INTO legacy_reviews(id,import_batch_id,source_file,sheet_name,source_row,raw_ocr_text,ocr_confidence,ocr_tokens_json,inherited_from,ocr_course_name,course_id,ocr_teacher_name,teacher_id,offering_id,category,comment,term,source_type,source_label,status,duplicate_group,created_at,reviewed_at,moderator_note)
SELECT id,import_batch_id,source_file,sheet_name,source_row,raw_ocr_text,ocr_confidence,ocr_tokens_json,inherited_from,ocr_course_name,course_id,ocr_teacher_name,teacher_id,offering_id,category,comment,term,source_type,source_label,status,duplicate_group,created_at,reviewed_at,moderator_note FROM legacy_reviews_identity_legacy;

ALTER TABLE legacy_review_moderation_events RENAME TO legacy_review_moderation_events_identity_legacy;
...
SELECT id,legacy_review_id,action,note,actor_session_id,created_at FROM legacy_review_moderation_events_identity_legacy;

ALTER TABLE catalog_requests RENAME TO catalog_requests_identity_legacy;
...
SELECT id,kind,course_code,course_name,category,teacher_name,teacher_name,department,note,pending_review_json,status,moderator_note,created_course_id,created_teacher_id,created_review_id,submitter_hash,created_at,reviewed_at FROM catalog_requests_identity_legacy;
...
INSERT INTO course_name_variants(course_id,name) SELECT id,name FROM courses;
...
DROP TABLE review_moderation_events_identity_legacy;
DROP TABLE legacy_review_moderation_events_identity_legacy;
DROP TABLE course_teachers_identity_legacy;
DROP TABLE offering_teachers_identity_legacy;
DROP TABLE catalog_requests_identity_legacy;
DROP TABLE reviews_identity_legacy;
DROP TABLE legacy_reviews_identity_legacy;
DROP TABLE offerings_identity_legacy;
DROP TABLE teachers_identity_legacy;
DROP TABLE courses_identity_legacy;
