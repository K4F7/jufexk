...
export function publicCourseDisplayName(name?: string | null): string {
  const trimmed = name?.trim() ?? "";
  return (
    publicPeSkillDisplayName(trimmed) ??
    publicEnglishFamilyLabel(trimmed) ??
    trimmed
  );
}
...
export function publicCourseDisplayNameSql(alias = "c"): string {
  ...
  const english = publicEnglishFamilySql(alias);
  return `CASE WHEN (${family}) IS NOT NULL THEN ${sqlStringLiteral(`${PE_PUBLIC_DISPLAY_PREFIX} [`)} || (${family}) || ']' WHEN (${english}) IS NOT NULL THEN (${english}) ELSE ${alias}.name END`;
}
...
export function publicRelationNameSortSql(
  ...
  return `${display},${alias}.code,${alias}.id,COALESCE(${teacherAlias}.name,''),COALESCE(${teacherAlias}.id,0)`;
}
...
export function publicRelationNameSortKeySql(
  ...
  const display = publicCourseDisplayNameSql(alias);
  return `printf('%s',${display}) || char(31) || printf('%s',${alias}.code) || char(31) || printf('%010d',${alias}.id) || char(31) || printf('%s',COALESCE(${teacherAlias}.name,'')) || char(31) || printf('%010d',COALESCE(${teacherAlias}.id,0))`;
}

/** PE families and 大学英语 I–IV / 1–4 merge to one public browse row. */
