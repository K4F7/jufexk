...
function percentile(values, fraction) {
  const nums = values.filter((value) => Number.isFinite(value)).sort((a, b) => a - b);
  if (nums.length === 0) return null;
  const position = (nums.length - 1) * fraction;
  const lower = Math.floor(position);
  const upper = Math.ceil(position);
  if (lower === upper) return round(nums[lower]);
  return round(nums[lower] + (nums[upper] - nums[lower]) * (position - lower));
}
...
async function runWalk(browser) {
  ...
  const summary = [...byId.entries()].map(([id, rows]) => {
    ...
    return {
      ...
      warmVisibleP50Ms: percentile(warm.map((row) => row.visibleMs), 0.5),
      warmVisibleP95Ms: percentile(warm.map((row) => row.visibleMs), 0.95),
      warmVisibleP99Ms: percentile(warm.map((row) => row.visibleMs), 0.99),
      warmVisibleMaxMs: Math.max(
        0,
        ...warm
          .map((row) => row.visibleMs)
          .filter((value) => Number.isFinite(value)),
      ),
      warmApiWallP50Ms: percentile(warm.map((row) => row.primaryWallMs), 0.5),
      warmApiWallP95Ms: percentile(warm.map((row) => row.primaryWallMs), 0.95),
      warmApiWallP99Ms: percentile(warm.map((row) => row.primaryWallMs), 0.99),
      warmApiWallMaxMs: Math.max(
        0,
        ...warm
          .map((row) => row.primaryWallMs)
          .filter((value) => Number.isFinite(value)),
      ),
