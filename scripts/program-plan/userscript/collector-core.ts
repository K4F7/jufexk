...
import { discoverTableId, findResultTable } from "../parse-table";
import type { ProgramPlanDimensions, ProgramPlanFilters } from "../query-plan";

export const COLLECTOR_SCHEMA_VERSION = "program-plan-capture-package/v1" as const;
...
export function buildPageRequest(
  ...
  page: number,
  context: { tableId?: string; clientWidth: number; formAction?: string },
) {
...
export function parsePageMetadata(bytes: Uint8Array, tableId?: string) {
  const html = new TextDecoder("gbk").decode(bytes);
  const declaredTableId = tableId || discoverTableId(html);
  const totalRecords = Number(
    (declaredTableId
      ? new RegExp(`showTotalRecord\\(\\s*['"]${declaredTableId}['"]\\s*,\\s*['"](\\d+)['"]`).exec(html)?.[1]
      : undefined)
    ?? /showTotalRecord\(\s*['"][^'"]+['"]\s*,\s*['"](\d+)['"]/.exec(html)?.[1]
    ?? "NaN",
  );
