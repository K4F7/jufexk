...
(() => {
  ...
  function parsePageMetadata(bytes) {
    ...
    if (!Number.isSafeInteger(totalRecords) || !pagination)
      throw new Error("page metadata missing");
  ...
  function isSessionExpired(response) {
    if (/\/cas\/login\.action(?:$|[?#])/.test(response.url))
      return true;
  ...
  function retryDelay(attempt, random) {
    const base = [2000, 5000, 15000][Math.min(attempt - 1, 2)];
  ...
  function retryAfterMilliseconds(value) {
    ...
    if (Number.isFinite(seconds))
      return Math.max(60000, seconds * 1000);
    const date = Date.parse(value);
    return Number.isFinite(date) ? Math.max(60000, date - Date.now()) : 60000;
  }
  ...
  class CollectorEngine {
    ...
    log(state, event, query, page, detail) {
      state.log.push({ at: this.dependencies.now(), queryId: query?.queryId, page, event, detail });
      if (state.log.length > 20000)
        state.log.splice(0, state.log.length - 20000);
    }
    async run(state) {
      state.phase = "running";
      ...
      for (const query of state.queries) {
        ...
        if (this.stopRequested)
          return this.transition(state, "stopped", "stopped");
        if (this.pauseRequested)
          return this.transition(state, "paused", "paused");
    ...
    async runQuery(state, query) {
      ...
      while (query.pageCount === 0 || query.nextPage <= query.pageCount) {
        if (this.stopRequested) {
          await this.transition(state, "stopped", "stopped", query, query.nextPage);
          return "halt";
        }
        if (this.pauseRequested) {
          await this.transition(state, "paused", "paused", query, query.nextPage);
          return "halt";
        }
        let response;
        for (let attempt = 1;attempt <= 4; attempt += 1) {
          try {
            ...
            if (isSessionExpired(response))
              throw new SessionExpiredError("login required");
            if (response.status === 429) {
              state.consecutiveServerFailures += 1;
              if (state.consecutiveServerFailures >= 2)
                return this.circuit(state, query, "consecutive HTTP 429");
            ...
            if (response.status >= 500) {
              state.consecutiveServerFailures += 1;
              if (state.consecutiveServerFailures >= 2)
                return this.circuit(state, query, `consecutive HTTP ${response.status}`);
        ...
        try {
          ...
          if (metadata.currentPage !== query.nextPage)
            throw new Error(`expected page ${query.nextPage}, received ${metadata.currentPage}`);
          ...
          if (projectedRecords > metadata.totalRecords)
            throw new Error("page rows exceed declared query total");
          if (metadata.currentPage < metadata.totalPages && projectedRecords >= metadata.totalRecords)
            throw new Error("query total reached before final page");
          if (metadata.currentPage === metadata.totalPages && projectedRecords !== metadata.totalRecords) {
  ...
  function stableJson(value) {
    ...
    if (value && typeof value === "object")
      return `{${Object.entries(value).sort(([a], [b]) => compareText(a, b)).map(([key, item]) => `${JSON.stringify(key)}:${stableJson(item)}`).join(",")}}`;
  ...
  function options(id) {
    ...
    if (!select)
      throw new Error(`missing source dictionary ${id}`);
  ...
  async function readSourceDictionary() {
    const content = { schemaVersion: "catalog-capture-package/v1", semesters: options("xnxq"), educationLevels: options("sel_pycc"), grades: options("sel_nj"), homeUnits: options("sel_cddwdm"), majors: options("sel_zydm"), capturedAt: new Date().toISOString() };
  ...
  function buildPlan(dictionary, mode) {
    ...
    if (mode === "single")
      return [makeQuery("main", current("xnxq"), current("sel_pycc"), current("sel_nj"))];
    const result = [];
    ...
    for (const semester of semesters)
      for (const level of levels)
        for (const grade of grades)
          result.push(makeQuery("main", semester, level, grade));
    ...
    if (base && major)
      result.push(makeQuery("counterexample", base.dimensions.semester, base.dimensions.educationLevel, base.dimensions.grade, { sel_zydm: major }, 1, "major"));
    if (base && homeUnit)
      result.push(makeQuery("counterexample", base.dimensions.semester, base.dimensions.educationLevel, base.dimensions.grade, { sel_cddwdm: homeUnit }, 2, "homeUnit"));
    return result;
  }
  function diffDictionary(before, after) {
    ...
    for (const name of ["semesters", "educationLevels", "grades", "homeUnits", "majors"]) {
      ...
      for (const [id, label] of newValues) {
        if (!oldValues.has(id))
          changes.push({ dictionary: name, kind: "added", id, after: label });
        else if (oldValues.get(id) !== label)
          changes.push({ dictionary: name, kind: "renamed", id, before: oldValues.get(id), after: label });
      }
      for (const [id, label] of oldValues)
        if (!newValues.has(id))
          changes.push({ dictionary: name, kind: "removed", id, before: label });
    }
    return changes;
  }
    ...
    for (const semester of dictionary.semesters)
      for (const level of dictionary.educationLevels)
        for (const grade of dictionary.grades)
          if (semesters.has(semester.id) || levels.has(level.id) || grades.has(grade.id))
            queries.push(makeQuery("supplemental", semester.id, level.id, grade.id, {}, 0, "dimensions"));
    for (const homeUnit of changed("homeUnits"))
      for (const semester of dictionary.semesters)
        for (const level of dictionary.educationLevels)
          for (const grade of dictionary.grades)
            queries.push(makeQuery("supplemental", semester.id, level.id, grade.id, { sel_cddwdm: homeUnit }, 0, `homeUnit-${homeUnit}`));
    for (const major of changed("majors"))
      for (const semester of dictionary.semesters)
        for (const level of dictionary.educationLevels)
          for (const grade of dictionary.grades)
            queries.push(makeQuery("supplemental", semester.id, level.id, grade.id, { sel_zydm: major }, 0, `major-${major}`));
    const unique = new Map(queries.map((query) => [query.queryId, query]));
    return [...unique.values()];
  }
  async function ensureDirectoryPermission() {
    if (!directory)
      throw new DirectoryUnavailableError("capture directory is not selected");
    ...
    if (permission !== "granted") {
      ...
      if (requested !== "granted")
        throw new DirectoryUnavailableError("capture directory permission lost");
    }
  }
  ...
  async function writeLiveArtifacts(next) {
    if (!await hasDirectoryPermission())
      return;
...
`;
  ...
  async function saveCheckpoint(next) {
    ...
    if (directory)
      await dbPut("directory", directory);
    try {
  ...
  function createEngine() {
    return new CollectorEngine({ request: requestPage, writeSnapshot: async (id, page, bytes) => writePath(`snapshots/${id}/page-${String(page).padStart(4, "0")}.html`, bytes), resetQuerySnapshots, saveCheckpoint, sleep: (ms) => new Promise((resolve) => setTimeout(resolve, ms)), now: () => new Date().toISOString(), random: Math.random });
  }
  async function planSourceSupplements() {
    if (!state || !frozenDictionary)
      return false;
    const current = await readSourceDictionary();
    if (current.sha256 === frozenDictionary.sha256)
      return false;
    ...
    state.log.push({ at: new Date().toISOString(), event: "source_changed", detail: JSON.stringify({ round: state.sourceChangeRounds, changes, addedQueries: additions.length }) });
    if (state.sourceChangeRounds > 3) {
      state.unresolvedSourceChanges = Math.max(1, state.unresolvedSourceChanges);
      state.log.push({ at: new Date().toISOString(), event: "coverage_exception", detail: "source dictionary changed in more than three consecutive rounds" });
      return false;
    }
...
`);
  ...
  async function planCounterexampleSupplements() {
    if (!state || !frozenDictionary || !needsCounterexampleSupplement())
      return false;
    const nonempty = state.queries.filter((query) => query.kind === "main" && query.status === "complete" && query.declaredRecordCount > 0);
    ...
    const additions = [];
    if (!hasMajor && frozenDictionary.majors[0])
      additions.push(makeQuery("counterexample", base.dimensions.semester, base.dimensions.educationLevel, base.dimensions.grade, { sel_zydm: frozenDictionary.majors[0].id }, 3, "nonempty-major"));
    if (!hasHomeUnit && frozenDictionary.homeUnits[0])
      additions.push(makeQuery("counterexample", base.dimensions.semester, base.dimensions.educationLevel, base.dimensions.grade, { sel_cddwdm: frozenDictionary.homeUnits[0].id }, 4, "nonempty-homeUnit"));
    ...
    state.phase = "paused";
    state.log.push({ at: new Date().toISOString(), event: "counterexample_supplemented", detail: `${unique.length} queries based on ${base.queryId}` });
    await saveCheckpoint(state);
    return true;
  }
  async function runToTerminal() {
    ...
    while (true) {
      ...
      if (state.phase !== "complete")
        break;
      if (await planCounterexampleSupplements())
        continue;
  ...
  async function start(mode) {
    if (!window.showDirectoryPicker)
      throw new Error("File System Access API unavailable; use Chrome/Edge");
    if (state && !window.confirm(`现有批次 ${state.batchId} 将保留在原目录。确认开始新的 ${mode} 批次？`))
      return;
...
`);
  ...
  async function retryExceptions() {
    ...
    if (!exceptions.length)
      throw new Error("no exception queries");
    ...
    state.phase = "paused";
    state.log.push({ at: new Date().toISOString(), event: "exception_retry_started", detail: `${exceptions.length} queries` });
    await saveCheckpoint(state);
    await resume();
  }
  function counts() {
    const statuses = {};
    for (const query of state?.queries ?? [])
      statuses[query.status] = (statuses[query.status] ?? 0) + 1;
    return statuses;
  }
  async function exportPackage() {
    if (!state || !frozenDictionary)
      throw new Error("no capture state");
    await ensureDirectoryPermission();
    const current = await readSourceDictionary();
    const sourceChanged = current.sha256 !== frozenDictionary.sha256 || (state.unresolvedSourceChanges ?? 0) > 0;
    state.log.push({ at: new Date().toISOString(), event: "export_complete", detail: sourceChanged ? "source_changed" : "validated" });
...
`;
    ...
    for (const query of state.queries)
      for (let page = 1;page <= query.pageCount && page < query.nextPage; page++) {
...
`);
  ...
  function showError(error) {
    const root = document.querySelector("#jufexk-catalog-panel");
    if (root)
      root.querySelector("[data-error]").textContent = error instanceof Error ? error.message : String(error);
  }
  function mount() {
    if (location.pathname !== "/student/wsxk.kcbcx10319.html" || document.querySelector("#jufexk-catalog-panel") || !document.querySelector("#ActionForm"))
      return;
    ...
    panel.addEventListener("click", (event) => {
      ...
      const run = async () => {
        showError("");
        if (action === "start")
          await start(panel.querySelector("[data-mode]").value);
        else if (action === "pause") {
          engine?.pause();
        } else if (action === "resume")
          await resume();
        else if (action === "retry")
          await retryExceptions();
        else if (action === "stop")
          engine?.stop();
    ...
    Promise.all([dbGet("state"), dbGet("directory"), dbGet("dictionary")]).then(async ([savedState, savedDirectory, savedDictionary]) => {
      state = savedState;
      directory = savedDirectory;
      frozenDictionary = savedDictionary;
      render();
      if ((state?.phase === "running" || state?.phase === "complete" && needsCounterexampleSupplement()) && await hasDirectoryPermission())
        runToTerminal().catch(showError);
    }).catch(showError);
  }
  mount();
})();
