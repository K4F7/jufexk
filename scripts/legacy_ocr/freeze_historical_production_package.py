...
FREEZE_CONTRACT = "legacy-historical-production-freeze-v2"
APPROVED_MANIFEST_SHA256 = (
    "edcf142cbd0380e734da0cde1923ee976ea9e25ab48147d0b78e218a64bb51af"
)
SOURCE_APPROVED_CATALOG_MANIFEST_SHA256 = (
    "198142a276de5e5ec88a1f290758d16922ac0174e6e4c57d135d7d6d1756fa9a"
)
...
EXPECTED_IMPORTABLE = 522
EXPECTED_RELATION_UNAVAILABLE = 419
...
OUTPUT_SCHEMAS = {
    ...
    "catalog-relation-unavailable.jsonl": "legacy-approved-review-v1",
...
def validate_target_catalog(
    ...
    *,
    ...
    if not catalog_manifest_path.is_file():
        raise FreezeError("missing approved catalog manifest")
    catalog_manifest_sha256 = sha256_file(catalog_manifest_path)
    if (
        expected_manifest_sha256 is not None
        and catalog_manifest_sha256 != expected_manifest_sha256
    ):
    ...
    if (
        catalog_manifest.get("schemaVersion")
        != "catalog-baseline-approved-manifest/v1"
        or catalog_manifest.get("status") != "package_ready"
        or catalog_manifest.get("contentSha256") != expected_content_sha256
    ):
        raise FreezeError("approved catalog manifest contract or content SHA-256 mismatch")

    manifest_counts = catalog_manifest.get("counts")
    if not isinstance(manifest_counts, dict):
        raise FreezeError("approved catalog manifest counts are missing")
    authoritative_counts = expected_counts or manifest_counts
    if any(manifest_counts.get(name) != count for name, count in authoritative_counts.items()):
        raise FreezeError("approved catalog manifest counts mismatch")

    artifact = catalog_manifest.get("artifact")
    if not isinstance(artifact, dict) or artifact.get("path") != "catalog-baseline.jsonl":
        raise FreezeError("approved catalog artifact descriptor is invalid")
    artifact_path = catalog_manifest_path.parent / "catalog-baseline.jsonl"
    if not artifact_path.is_file():
        raise FreezeError("missing approved catalog artifact")
    artifact_sha256 = sha256_file(artifact_path)
    authoritative_artifact_sha256 = expected_artifact_sha256 or artifact.get("sha256")
    if (
        artifact_sha256 != authoritative_artifact_sha256
        or artifact.get("sha256") != authoritative_artifact_sha256
        or artifact.get("bytes") != artifact_path.stat().st_size
        or artifact.get("records") != manifest_counts.get("totalRecords")
    ):
        raise FreezeError("approved catalog artifact hash, size, or record count mismatch")
    ...
    with artifact_path.open(encoding="utf-8") as stream:
        for line_number, line in enumerate(stream, 1):
            if not line.strip():
                raise FreezeError(
                    f"catalog-baseline.jsonl:{line_number} contains a blank line"
                )
            row = json.loads(line)
            if (
                not isinstance(row, dict)
                or row.get("schemaVersion")
                != "catalog-baseline-approved-record/v1"
                or row.get("recordType") not in {"course", "teacher", "relation"}
                or not isinstance(row.get("value"), dict)
            ):
                raise FreezeError(
                    f"catalog-baseline.jsonl:{line_number} has an invalid record"
                )
            target = {
                "course": courses,
                "teacher": teachers,
    ...
    actual_counts = {
        ...
        "totalRecords": len(courses) + len(teachers) + len(relations),
    }
    if actual_counts != authoritative_counts:
        raise FreezeError("approved catalog artifact counts mismatch")
...
def _freeze(
    ...
    *,
    ...
    expected_catalog_artifact_sha256: str | None = None,
    expected_catalog_counts: dict[str, int] | None = None,
    expected_importable: int = EXPECTED_IMPORTABLE,
    expected_relation_unavailable: int = EXPECTED_RELATION_UNAVAILABLE,
) -> dict[str, Any]:
    ...
    source_catalog_manifest_sha256 = source_manifest.get(
        "approved_catalog_manifest_sha256"
    )
    if (
        ...
        raise FreezeError("source package approved catalog lineage mismatch")
    ...
    (
        target_course_codes,
        target_teacher_labels,
        target_relation_keys,
        catalog_manifest_sha256,
        catalog_artifact_sha256,
    ) = validate_target_catalog(
        catalog_manifest_path,
        expected_manifest_sha256=expected_catalog_manifest_sha256,
        expected_content_sha256=expected_catalog_content_sha256,
        expected_artifact_sha256=expected_catalog_artifact_sha256,
    ...
    approved_rows = source_rows["approved-legacy-reviews.jsonl"]
    missing_target_identity = [
        (index, row)
        for index, row in enumerate(approved_rows, 1)
        if row["catalog_course_code"] not in target_course_codes
        or row["catalog_teacher_label"] not in target_teacher_labels
    ]
    if missing_target_identity:
        index, _ = missing_target_identity[0]
        raise FreezeError(
            f"approved-legacy-reviews.jsonl:{index} lacks a v2 course or teacher identity"
        )
    importable_rows = [
        row
        for row in approved_rows
        if (row["catalog_course_code"], row["catalog_teacher_label"])
        in target_relation_keys
    ]
    relation_unavailable_rows = [
        row
        for row in approved_rows
        if (row["catalog_course_code"], row["catalog_teacher_label"])
        not in target_relation_keys
    ]
    if (
        len(importable_rows) != expected_importable
        or len(relation_unavailable_rows) != expected_relation_unavailable
    ):
        raise FreezeError("v2 catalog relation split does not match the approved scope")

    partition_rows: dict[str, list[dict[str, Any]]] = {
        "importable-legacy-reviews.jsonl": importable_rows,
        "catalog-relation-unavailable.jsonl": relation_unavailable_rows,
    }
    partition_rows.update(
        {target: source_rows[source] for source, target in PARTITIONS.items()}
    )
    ...
    lineage = {
        ...
        "approvedCatalogArtifactSha256": catalog_artifact_sha256,
        "approvedPackageManifestSha256": source_manifest_sha256,
        "approvedPackageContract": SOURCE_CONTRACT,
        "sourceApprovedCatalogManifestSha256": source_catalog_manifest_sha256,
    ...
    freeze_counts = {
        ...
        "catalogRelationUnavailable": len(
            partition_rows["catalog-relation-unavailable.jsonl"]
        ),
...
- v2 正式任课关系不可用：{freeze_counts['catalogRelationUnavailable']}
...
`{freeze_counts['sourceRows']} + {freeze_counts['fanOutExtraRows']} = {freeze_counts['dispositionRows']} = {freeze_counts['importable']} + {freeze_counts['catalogRelationUnavailable']} + {freeze_counts['catalogIdentityUnresolved']} + {freeze_counts['evidenceConflict']} + {freeze_counts['blank']} + {freeze_counts['otherExcluded']}`。{freeze_counts['sourceRows']} 个来源评价全部闭合，fan-out 仅增加终态行；每个唯一 `review_id` 只写入一个分区。

## 资格与安全门禁

- 可导入行全部通过 v2 既有课程、教师、正式任课关系和非空正文校验。
- {freeze_counts['catalogRelationUnavailable']} 条已批准评价因 v2 缺少正式任课关系保持物理隔离，由 #111 独立处理，不进入本批生产 artifact。
...
- 唯一权威输入是 #24 关闭时的 `historical-approved-package-v1`；其 1,338 条 API-ready 来源因 41 条人工 fan-out 决策形成 1,379 条终态处置行。原批准包的 v1 目录 lineage 与本次 v2 目标 lineage 分别固定校验，不将二者假定为同一 manifest。
- 同目录的后续包或其他候选包即使包含更多可绑定记录，也因 manifest 哈希不同而被拒绝，冻结过程不拼接来源；941 是批准总集，本批生产集固定为 522。
...
def freeze(
    ...
    return _freeze(
        ...
        expected_catalog_manifest_sha256=APPROVED_CATALOG_MANIFEST_SHA256,
        expected_catalog_artifact_sha256=APPROVED_CATALOG_ARTIFACT_SHA256,
        expected_catalog_counts=EXPECTED_CATALOG_COUNTS,
        expected_importable=EXPECTED_IMPORTABLE,
        expected_relation_unavailable=EXPECTED_RELATION_UNAVAILABLE,
    )
